...
- `scripts/gaoyou_cropland_binary_enhanced_2020_2025.js`：耕地/非耕地增强二分类脚本；融合Sentinel-2 NDVI/EVI/LSWI/MNDWI/NDBI/NDRE、Sentinel-1 VV/VH/VV−VH/VV/VH时序和物候特征，默认RF，支持一次运行多个耕地概率阈值优化，输出OA、Kappa、PA、UA、F1、混淆矩阵和耕地面积表。
...
- Google Drive文件夹：`GEE_Gaoyou_Model_Comparison`
- 18组实验的OA、Kappa、Macro-F1、各类别Precision/Recall/F1和混淆矩阵CSV
- 9个空间验证模型结果的多波段GeoTIFF
- RF+SAVI空间验证候选分类GeoTIFF
